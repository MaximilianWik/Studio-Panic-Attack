 ...
 *
 * We probe for these on disk and emit:
 *   - per-asset srcset strings + lqip + intrinsic width/height
 *   - a flat OPTIMIZED_INDEX keyed by original URL so non-asset code paths
 *     (THREE.js textures, manual <img> tags) can pick the right WebP variant
 *     for a given target width via the `pickOptimized()` helper.
 *
 * If the siblings don't exist (optimize hasn't been run yet), the asset
 * falls back to a plain <img src=originalUrl>.
 *
...
catch { console.log('[manifest] sharp not installed — width/height fields will be omitted.'); }
 ...
 * metadata (per-width WebP urls, AVIF, LQIP). Returns null if no siblings
 * exist.
 */
async function readOptimized(absImagePath, urlBase) {
  ...
  for (const w of WEBP_WIDTHS) {
    if (await exists(baseAbs + '.' + w + '.webp')) {
      const u = `${baseUrl}.${w}.webp`;
      webp['w' + w] = u;
      webpEntries.push(`${u} ${w}w`);
    }
  }

  // AVIF (single width).
  const avifAbs = baseAbs + '.' + AVIF_WIDTH + '.avif';
  const avifUrl = (await exists(avifAbs)) ? `${baseUrl}.${AVIF_WIDTH}.avif` : null;
  const avif = avifUrl ? `${avifUrl} ${AVIF_WIDTH}w` : null;
  ...
  return {
    ...
    avifSrcset: avif,
    lqip,
    // For the OPTIMIZED_INDEX (raw urls per width).
    raw: {
      webp480: webp.w480 ?? null,
      webp1080: webp.w1080 ?? null,
      webp1920: webp.w1920 ?? null,
      avif1080: avifUrl,
      lqip: lqip,
    },
  };
...
/** Mutated as we walk assets — keyed by original URL. */
const OPTIMIZED_INDEX = {};

/**
 * Read intrinsic image dimensions via sharp metadata. Returns null on any
 * error (corrupted file, unsupported format, sharp not installed).
 */
async function readDimensions(abs) {
...
async function toAsset(folderRel, filename) {
  ...
  if (isImg && RASTER_RE.test(filename)) {
    ...
    const [opt, dims] = await Promise.all([
      readOptimized(abs, url),
      readDimensions(abs),
    ]);
    if (opt) {
      ...
      // Index by original url for non-asset code paths (THREE textures etc).
      OPTIMIZED_INDEX[url] = opt.raw;
    }
    if (dims) {
...
const body =
  ...
  'export const OPTIMIZED_INDEX: Record<string, OptimizedSiblings> = ' +
    JSON.stringify(OPTIMIZED_INDEX, null, 2) + ';\n';
...
console.log('  OPTIMIZED_INDEX entries:', Object.keys(OPTIMIZED_INDEX).length);
